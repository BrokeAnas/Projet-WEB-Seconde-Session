...
import { Paiement } from '../../services/api/models/paiement.model';
import { PaiementFormPageComponent } from '../paiement-form-page/paiement-form-page.component';
...
@Component({
  ...
  styleUrl: './paiements-page.component.scss'
})
export class PaiementsPageComponent implements OnInit {
  ...
  nouveau(): void {
    const ref = this.dialog.open(PaiementFormPageComponent, { width: '520px', maxWidth: '95vw' });
    ref.afterClosed().subscribe(ok => { if (ok) this.reload(); });
  }

  modifier(p: Paiement): void {
    const ref = this.dialog.open(PaiementFormPageComponent, { width: '520px', maxWidth: '95vw', data: p });
    ref.afterClosed().subscribe(ok => { if (ok) this.reload(); });
  }
